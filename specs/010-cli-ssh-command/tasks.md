...
- [X] T002 [P] Survey reusable patterns read-only in `crates/cli/src/commands/start.rs`, `crates/cli/src/commands/new.rs`, `crates/cli/src/privilege.rs`, `crates/cli/src/error.rs` (escalation flow, `Select` + `prompt_render_config`, `resolve_name`/`validate_name`, `backend_path` lookup, triplet errors, child exit-code convention)
...
- [X] T017 Run `cargo fmt --all -- --check` from repository root and apply formatting to touched files (`crates/sdk/src/domain/microvm.rs`, `crates/sdk/src/lib.rs`, `crates/sdk/src/manager.rs`, `crates/cli/src/cli.rs`, `crates/cli/src/commands/mod.rs`, `crates/cli/src/commands/ssh.rs`, `crates/cli/src/error.rs`, test files)
