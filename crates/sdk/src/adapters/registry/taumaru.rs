...
fn validate_image(distribution: &Distribution, image: &DistributionImage) -> Result<(), SdkError> {
    ...
    if image.format != "ext4" {
        return Err(SdkError::invalid_metadata(
            artifact.clone(),
            "distribution images must use the ext4 format",
        ));
    }
    validate_required_text(
    ...
    if image.filesystem.filesystem_type != "ext4" {
        return Err(SdkError::invalid_metadata(
            artifact.clone(),
            "distribution images must declare an ext4 filesystem",
        ));
    }
